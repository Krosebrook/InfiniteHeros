    ...
    return (
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/80 p-4">
             <div className="bg-white border-[6px] border-black p-6 max-w-md w-full shadow-[12px_12px_0px_white] animate-in fade-in zoom-in duration-300 relative">
                <h2 className="font-comic text-3xl mb-4 border-b-4 border-black pb-2 text-black">{t(lang, "SYSTEM_CONFIG")}</h2>
                ...
                <div className="mb-6">
                    <label className="flex items-center gap-3 font-comic text-xl cursor-pointer p-2 border-2 border-transparent hover:border-black transition-all">
                        ...
                        <span>{t(lang, "AUTO_READ")}</span>
                    </label>
                </div>

                <div className="mb-6">
                    <label className="block font-bold mb-2 font-comic text-xl">{t(lang, "VOICE_SPEED")}: {localSettings.playbackSpeed}x</label>
                ...
                <div className="mb-8">
                     <label className="block font-bold mb-2 font-comic text-xl">{t(lang, "ACHIEVEMENTS")}</label>
                ...
                <div className="flex gap-4">
                    <button onClick={onClose} className="flex-1 py-3 border-4 border-black font-bold font-comic text-xl uppercase">{t(lang, "CANCEL")}</button>
                    <button onClick={handleSave} className="flex-1 py-3 border-4 border-black bg-green-400 font-bold font-comic text-xl uppercase shadow-[4px_4px_0px_black]">{t(lang, "SAVE_SETTINGS")}</button>
                </div>
             </div>
        </div>
    );
