    ...
    return (
        <div className="fixed inset-0 z-[600] flex items-center justify-center bg-black/80 p-4">
             <div className="bg-white border-4 border-black p-6 max-w-sm w-full shadow-[10px_10px_0px_white] animate-in fade-in zoom-in duration-300">
                <h2 className="font-comic text-3xl mb-4 border-b-4 border-black pb-2">{t(lang, "EXPORT_COMIC")}</h2>
                
                <div className="mb-4">
                    <label className="block font-bold mb-1 font-comic text-lg">{t(lang, "FORMAT")}</label>
                ...
                <div className="mb-4">
                     <label className="block font-bold mb-1 font-comic text-lg">{t(lang, "SIZE")}</label>
                     <select value={scale} onChange={e => setScale(parseFloat(e.target.value))} className="w-full border-2 border-black p-2 font-comic text-lg bg-white">
                         <option value={1}>{t(lang, "ORIGINAL_SIZE")}</option>
                         <option value={0.75}>{t(lang, "MEDIUM_SIZE")}</option>
                         <option value={0.5}>{t(lang, "WEB_FRIENDLY_SIZE")}</option>
                     </select>
                </div>

                <div className="flex gap-2 mt-6">
                    <button onClick={onClose} disabled={isExporting} className="flex-1 py-3 border-2 border-black font-bold hover:bg-gray-200 font-comic text-xl">{t(lang, "CANCEL")}</button>
